...
public class PracticeAndCategorySearchPage extends ParentPage {
	...
	public PracticeAndCategorySearchPage(TreeBean parent, List<TreeBean>parents) {
		...
		}}); add(topPanel); topPanel.setVisible(true);
		...
		ListView<TreeBean> categoryList = new PropertyListView<TreeBean>("category", parents) {
			...
			protected void populateItem(ListItem<TreeBean> item) {
				if (!item.getModel().getObject().getName().equals(parent.getName())) {
					...
					item.add(new ManagePracticeNamePanel("categoryName", item.getModel()));
		...
		form.add(new CheckBoxMultipleChoice<String>("keywordCheck", searchModel, selectList) {
			...
			protected void onInitialize() {
				...
				add(new AjaxFormChoiceComponentUpdatingBehavior() {
					...
					@Override
					protected void onUpdate(AjaxRequestTarget target) {target.add();}});}});
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		val tagList = practiceService.selectAllTagList(parent.getName());
		val checkList = new ArrayList<String>();
		val checkGroup = new CheckGroup<>("checkGroup", checkList);

			// 列を制御するリストビュー split(lis, int 要素数)
		...
		AjaxButton keySearchResultPageLink = new AjaxButton("toKeySearchResultLinkPage", form) {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				if (searchModel.getObject().size() > 0) {
					setResponsePage(new PracticeAndCategorySearchResultPage(parent, parents, searchModel.getObject(), itemCategory));
			}else {
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		AjaxButton tagSearchResultPageLink = new AjaxButton("toTagSearchResultLinkPage", form) {
			...
			protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
				...
				if (checkList.size() > 0) {
					setResponsePage(new PracticeAndCategorySearchResultPage(parent, parents, checkList, itemCategory));
				} else{
